...
def launch_setup(context, *args, **kwargs):
    ...
    linear_motor = LaunchConfiguration('linear_motor', default=False)
    ...
    xyz = attach_xyz.perform(context).split(' ')
    rpy = attach_rpy.perform(context).split(' ')
    args = [
        xyz[0], xyz[1], xyz[2],    # X, Y, Z
        rpy[0], rpy[1], rpy[2],    # roll, pitch, yaw
